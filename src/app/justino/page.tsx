    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="bg-white rounded-t-2xl shadow-md p-4">
        <div className="flex flex-col items-center gap-4 text-center">
          <Image src={logo} alt="Logo Justino" width={100} height={100} />
          <h1 className="text-2xl font-bold">Seu Justino</h1>
        </div>
        ...
        {showSobre ? (
          <>
            <p className="text-sm text-center">
              O Seu Justino é o lugar ideal para quem busca diversão, boa comida e bons drinks em um só lugar.
            </p>
              ...
              <Carousel responsive={responsive} arrows={false} showDots swipeable itemClass="px-2">
                {[ambiente1, ambiente2, ambiente3, ambiente4].map((img, i) => (
                  <div key={i} onClick={() => openImage(img.src)} className="cursor-pointer">
                    <Image src={img} alt={`Ambiente ${i + 1}`} className="rounded-lg object-cover w-full h-[200px]" />
                  </div>
                ))}
              </Carousel>
            </div>
            ...
            <div className="mt-6">
              ...
              <Carousel responsive={responsive} arrows={false} showDots swipeable itemClass="px-2">
                {[gastro1, gastro2, gastro3, gastro4].map((img, i) => (
                  <div key={i} onClick={() => openImage(img.src)} className="cursor-pointer">
                    <Image src={img} alt={`Gastronomia ${i + 1}`} className="rounded-lg object-cover w-full h-[200px]" />
                  </div>
                ))}
              </Carousel>
            </div>
            ...
            <div className="mt-6">
              ...
              <Carousel responsive={responsive} arrows={false} showDots swipeable itemClass="px-2">
                {[drink1, drink2, drink3, drink4].map((img, i) => (
                  <div key={i} onClick={() => openImage(img.src)} className="cursor-pointer">
                    <Image src={img} alt={`Drink ${i + 1}`} className="rounded-lg object-cover w-full h-[200px]" />
                  </div>
                ))}
              </Carousel>
            </div>
